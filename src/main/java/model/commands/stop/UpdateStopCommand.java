...
import ua.kpi.epam.transport.commands.validators.stop.UpdateStopCommandValidator;
...
public class UpdateStopCommand implements StopCommand {
    ...
    private static final String UPDATE_STOP_SUCCESSFULL_MSG = "UpdateStopSuccessfull";
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) {
        ...
        stopDao.update(new Stop(Integer.valueOf(request
                .getParameter(ID_ATTRIBUTE)), request
                .getParameter(NAME_ATTRIBUTE), request
                .getParameter(ADDRESS_ATTRIBUTE)));

        request.setAttribute(RESULT_ATTRIBUTE, LocalizationHelper.getInstanse()
                .getLocalizedMessage(request, UPDATE_STOP_SUCCESSFULL_MSG));
        try {
